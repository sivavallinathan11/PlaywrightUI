...
import { Accommodation, BestMateMember, DEV_BestMateMember, TestingEnvironment } from "../data/users";
...
test('Add discount to a single booking where assigned guest is an  existing MEMBER', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3803-AddDiscountToBookingUsingExistingMember");
    ...
    // Verify that selected discount is applied then member discount is removed and membership fee is still displayed.
    var discountDetails = await reservepay.AddDiscountAndVerifyPaymentDetails(bookingDetails, guestDetails);
    bookingDetails = discountDetails[0];

    // Cancel Reservation.
    await reservepay.CancelReservation();

    // Verify accommodation details after discount is added.
    await booking.VerifyAccommodationPriceAfterDiscount(bookingDetails, guestDetails);
