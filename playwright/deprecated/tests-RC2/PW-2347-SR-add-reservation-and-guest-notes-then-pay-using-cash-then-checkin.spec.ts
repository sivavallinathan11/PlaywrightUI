...
import { Notes } from "../data/users";
import { BaseSteps } from "../models/BaseSteps";

test('Add reservation or guest notes then pay reservation using cash then checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2347-SR-add-reservation-and-guest-notes-then-pay-using-cash-then-checkin");
    ...
    await managebooking.AddReservationNotes(Notes.reservation);

    // Add guest note.
    await managebooking.AddGuestNotes(Notes.guest);
