...
test('Add membership of reservation claim all benefits then pay using cash-cc and checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2363-SR-buy-membership-for-reservation-pay-using-split-payment-using-cash-cc-then-checkin");
    ...
    var reservationNumber = await apiHelper.CreateReservation("Buy member", "Booking Number");
    ...
    var paymentData = await managebooking.SelectPaymentMethodandPercentage("Cash", mbData.totalBalance, "50");
    ...
    // Select payment method and percentage.
    var paymentData = await checkin.SelectPaymentMethodandPercentage("Credit Card", paymentData.balance, "100");

    // Click process payment.
    await checkin.ClickProcessPayment();

    // Verify payment made.
    await checkin.VerifyLatestPaymentMade(paymentData, "Credit Card");
