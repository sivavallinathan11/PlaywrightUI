...
test('Renew expired member outside grace period then pay using cash and checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2366-SR-renew-expired-member-outside-grace-period-then-pay-using-cash-and-checkin");
    ...
    await apiHelper.CreateAndRegisterMember(true);

    // Expire existing member.
    var memberInfo = await apiHelper.SearchAndExpireMember("Year", 1, true);
