...
test('Partially pay reservation using cash in MB then pay remaining using cc in checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2369-SR-partially-pay-reservation-using-cash-in-MB-then-pay-remaining-using-cc-in-checkin");
