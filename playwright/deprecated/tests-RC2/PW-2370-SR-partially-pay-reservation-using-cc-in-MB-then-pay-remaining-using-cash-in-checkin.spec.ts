...
test('Partially pay reservation using CC in MB then pay remaining using cash in checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2370-SR-partially-pay-reservation-using-cc-in-MB-then-pay-remaining-using-cash-in-checkin");
    ...
    // Select payment method and percentage.
    var newPaymentData = await checkin.SelectPaymentMethodandPercentage("Cash", paymentData.balance, "100");

    // Click process payment.
    await checkin.ClickProcessPayment();

    // Verify payment made.
    await checkin.VerifyLatestPaymentMade(newPaymentData, "Cash");
