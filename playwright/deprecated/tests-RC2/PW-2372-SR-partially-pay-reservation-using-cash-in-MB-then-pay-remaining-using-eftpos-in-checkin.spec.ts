...
test('Partially pay reservation using cash in MB then pay remaining using eftpos in checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2372-SR-partially-pay-reservation-using-cash-in-MB-then-pay-remaining-using-eftpos-in-checkin");
    ...
    // Verify payment made.
    await managebooking.VerifyPaymentMadeInManageBooking(paymentData, "Cash");
