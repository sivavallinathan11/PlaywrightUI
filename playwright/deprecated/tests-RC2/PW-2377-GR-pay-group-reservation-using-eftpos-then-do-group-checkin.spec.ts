...
test('Select reservation then change room and pay using eftpos then checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2377-GR-pay-group-reservation-using-eftpos-then-do-group-checkin");
    ...
    //const groupBookingNumber = payGroupBookingUsingEFTPOS.groupBookingNumber;
    ...
    const dashboardData = await dashboard.SelectSpecificGroupUsingReservationNumber(groupBookingNumber, 
    ...
    var paymentData = await manageGroup.SelectPaymentMethodAndVerifyPaymentDetails("EFTPOS", dashboardData);
    ...
    await checkinGroup.VerifyStayCostAndPaymentDetails(paymentData, dashboardData);
