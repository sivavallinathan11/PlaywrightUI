...
import { LesserOffer, MateMember } from "../data/users";
...
test('Add member to all bookings and add offer less than GDay pricing then apply Gday pricing', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-4326-AddMemberToAllBookingsAndAddOfferNotBetterThanGdayPricingThenApplyGDay");
    ...
    var offerList = LesserOffer + "|" + LesserOffer;
    var processOfferTypes = "Apply|Apply";
    var afterOfferDetails = await offers.SelectAndProcessOfferFromViewOffersModalAfterGuestIsAssigned(bookingDetails, guestDetails, 
        "Accept Offers", offerList, processOfferTypes);
