...
import { LesserOffer, UpsellGuestData } from "../data/users";
...
test('Add offer less than GDay Pricing to all bookings then join gday', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-4327-AddOfferLessThanGDayPricingToAllBookingsThenJoinGDay");
    ...
    var newCustomerDetails = [UpsellGuestData, UpsellGuestData];
    ...
    var offerTypeList = "Lesser|Lesser";
