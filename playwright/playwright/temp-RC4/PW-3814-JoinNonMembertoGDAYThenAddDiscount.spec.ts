...
test('Join a non-member guest to Gday then add discount', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3814-JoinNonMembertoGDAYThenAddDiscount");
