...
test('Buy membership of less than 500 booking cost claim all benefits then pay using cash and checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2355-SR-buy-membership-of-booking-cost-less-than-500-pay-using-cash-then-checkin");
    ...
    var paymentData = await managebooking.SelectPaymentMethodandPercentage("EFTPOS", mbData.totalBalance, "100");
