...
test('Add membership for paid reservation claim all benefits then pay using cc and checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2360-SR-buy-membership-for-paid-reservation-and-pay-using-cc-then-checkin");
    ...
    var paymentData = await managebooking.SelectPaymentMethodandPercentage("Credit Card", mbData.totalBalance, "100");

    // Click process payment.
    await managebooking.ClickProcessPayment();

    // Verify payment made.
    await managebooking.VerifyPaymentMadeInManageBooking(paymentData, "Credit Card");
