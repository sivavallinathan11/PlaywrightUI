...
test('Add membership of paid reservation claim all benefits then pay using cash-eftpos and checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2364-SR-buy-membership-for-paid-reservation-pay-using-split-payment-using-cash-eftpos-then-checkin");
    ...
    // Select payment method and percentage.
    var paymentData = await checkin.SelectPaymentMethodandPercentage("EFTPOS", paymentData.balance, "100");
