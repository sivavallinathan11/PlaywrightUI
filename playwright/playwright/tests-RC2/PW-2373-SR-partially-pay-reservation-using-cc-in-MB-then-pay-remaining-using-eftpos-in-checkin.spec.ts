...
test('Partially pay reservation using CC in MB then pay remaining using eftpos in checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2373-SR-partially-pay-reservation-using-cc-in-MB-then-pay-remaining-using-eftpos-in-checkin");
    ...
    // Select payment method and percentage.
    var newPaymentData = await checkin.SelectPaymentMethodandPercentage("EFTPOS", paymentData.balance, "100");

    // Click process payment.
    await checkin.ClickProcessPayment();

    // Verify payment made.
    await checkin.VerifyLatestPaymentMade(newPaymentData, "EFTPOS");
