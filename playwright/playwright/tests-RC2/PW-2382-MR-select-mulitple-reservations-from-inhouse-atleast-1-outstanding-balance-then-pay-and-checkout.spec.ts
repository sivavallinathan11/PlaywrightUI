...
test('Select multiple reservation from in-house with at least 1 outstanding balance then pay and checkout', async ({page, request}) => {
    ...
    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2382-MR-select-mulitple-reservations-from-inhouse-atleast-1-outstanding-balance-then-pay-and-checkout");
    ...
    var reservations = await apiHelper.CreateMultipleReservations("Multiple Inhouse", "Booking Number", 3, 1);
    ...
    await dashboard.VerifyConfirmCheckoutModal(reservations.length, true);
