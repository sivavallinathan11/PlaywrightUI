...
test("Checkin group reservation with 1 booking of past and future dated amd 1 booking of future dated", async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2387-SR-checkin-reservation-with-past-dated-checkin-and-future-dated-checkout");
    ...
    const groupBookingNumber = await apiHelper.CreateGroupReservation("non member", "booking number", "-1", "1", "1", "2");
    ...
    //var groupBookingNumber = groupBookingPastFutureFuture.groupBookingNumber;
    ...
    await manageGroup.VerifyDisabledGroupCheckOut();

    // Verify expected on-hover message.
    await manageGroup.VerifyHoverOnUnpaidPastDatedGroupReservation();
