...
test('Filter the search results to 24hrs', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 

    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2391-Filter-the-search-results-to-24hrs");
    ...
    // Filter to 24 hrs and verify if loading icon were displayed
    await dashboard.ClickUpcomingFilterTabAndVerifyLoadingIcon("24");

    // Wait for loading icon of filter to disappear
    await dashboard.waitForLoadingIconToDisappear("24");

    // This will verify the records in the list
    await dashboard.verifyFilteredAccommodation("24");
