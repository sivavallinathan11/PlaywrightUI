...
test('Add membership of reservation claim all benefits then pay using cash-cc and checkin', async ({page, request}) => {
    ...
    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2399-SR-buy-membership-for-reservation-pay-using-split-payment-using-cc-cc-then-checkin");
    ...
    var paymentData = await managebooking.SelectPaymentMethodandPercentage("Credit Card", mbData.totalBalance, "50");

    // Click process payment.
    await managebooking.ClickProcessPayment();

    // Verify payment made.
    var firstCCSurcharge = await managebooking.VerifyPaymentMadeInManageBooking(paymentData, "Credit Card");
    ...
    // Verify payment made.
    await checkin.VerifyLatestPaymentMade(newPaymentData, "Credit Card", firstCCSurcharge);
