...
test('Add membership of more than 500 booking cost claim all benefits then pay using cash and checkin', async ({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps(); 
     
    // Initialize Test
    var testDetails = await baseSteps.InitializeTestResultDirectory("PW-2531-SR-add-membership-of-booking-cost-more-than-500-pay-using-cash-then-checkin");
    ...
    //#region Start test.
    //Create a reservation with more than $500 balance.
    var reservationNumber = await apiHelper.CreateReservation("More Than 500", "Booking Number");
    ...
    // Select reservation based on the reservation number.
    ...
    var memberDiscount = await managebooking.VerifyMembershipFees(dashboardData, dashboardData.paymentType);

    // Verify new member details.
    var memberTier = await managebooking.VerifyNewMemberDetails();
    ...
    memberDiscount + "|" + paymentDetails + "|" + memberTier, dashboardData);
