...
test('Create and pay an individual booking partially using EFTPOS', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3119-CreateIndividualBookingAndPayPartiallyUsingEFTPOS");
    ...
    // Pay using EFTPOS partially. Payment percentage set to 'Random' to check every percentage.
