...
import { Accommodation, BestMateMember, DEV_BestMateMember, LesserOffer, TestingEnvironment } from "../data/users";
...
test('Add an offer THAT IS LESS THAN GDAY PRICING then assign an existing member to the booking', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3799-AddOfferLessThanGDayPricingThenAssignExistingMember");
