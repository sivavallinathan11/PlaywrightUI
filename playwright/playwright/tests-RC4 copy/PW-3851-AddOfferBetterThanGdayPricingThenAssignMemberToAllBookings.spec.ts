...
test('Add offer better than GDay pricing then assign member to all bookings', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3851-AddOfferBetterThanGdayPricingThenAssignMemberToAllBookings");
