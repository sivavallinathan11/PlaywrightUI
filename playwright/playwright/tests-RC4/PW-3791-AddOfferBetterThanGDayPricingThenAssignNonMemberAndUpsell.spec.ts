...
import { BetterOffer, UpsellGuestData } from "../data/users";
...
test('Add an offer better than Gday Pricing then assign a non-member to the booking the upsell', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3791-AddOfferBetterThanGDayPricingThenAssignNonMemberAndUpsell");
