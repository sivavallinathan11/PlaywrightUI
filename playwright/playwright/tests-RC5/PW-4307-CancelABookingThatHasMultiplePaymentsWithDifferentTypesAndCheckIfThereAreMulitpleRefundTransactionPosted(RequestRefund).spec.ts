...
test('Cancel a booking that has multiple payments with different types and check if there are multiple refund transaction posted (Request Refund)', async({page, request}) =>{
    // Set base object.
    ...
    await apiHelper.CreateSingleOrMultiplePayments(bookingNumber, "mixed", 5, 60);
    ...
    await processRefund.VerifyProcessRefundSection("mixed");

    // Verify Park Refund Details
    await processRefund.VerifyParkRefundtDetails(stayCostDetails);
    ...
    // Verify Booking in PMS
    await apiHelper.CheckBookingInPMS(bookingDetails.ReservationNumber, stayCostDetails, "Test");
