...
import { Accommodation, GuestDataPayLater } from "../../data/users";
...
test('Create an individual booking then pay it later', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3112-CreateIndividualBookingThenPayLater");
    ...
    const customerDetails = GuestDataPayLater;
    ...
    // Process payment.
    var paymentDetails = await reservepay.ManagePayment(bookingDetails, guestDetails, "Skip");
