...
test('Create an individual booking then upsell and pay using any payment method', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3113-CreateIndividualBookingthenUpsellAndPayUsingAnyPaymentMethod");
    ...
    // Pay using any mode of payment. If payment has no percentage it will be classified as specific payment.
