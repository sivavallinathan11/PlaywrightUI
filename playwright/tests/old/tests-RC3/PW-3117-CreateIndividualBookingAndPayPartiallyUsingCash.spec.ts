...
import { TestingEnvironment, MateMember, DEV_BestMateMember } from "../../data/users";
...
test('Create and pay an individual booking partially using cash', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3117-CreateIndividualBookingAndPayPartiallyUsingCash");
    ...
    // Pay using cash partially. Payment percentage set to 'Random' to check every percentage.
    var paymentDetails = await reservepay.ManagePayment(bookingDetails, guestDetails, "Cash", "Random");
