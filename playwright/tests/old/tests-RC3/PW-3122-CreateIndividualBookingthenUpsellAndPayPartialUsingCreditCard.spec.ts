...
import { Accommodation, UpsellGuestData, UpsellGuestData500 } from "../../data/users";
...
test('Create an individual booking then upsell and pay partially using credit card', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3122-CreateIndividualBookingthenUpsellAndPayPartialUsingCreditCard");
    ...
    // Pay using credit card partially. If payment has no percentage it will be classified as specific payment.
