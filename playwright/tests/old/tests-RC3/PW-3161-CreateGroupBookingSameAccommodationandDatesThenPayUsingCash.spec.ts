...
import { SameAccommodationAndDatesGroupData } from "../../data/users";
...
test('Create a group booking with the same accommodation and dates then pay using cash', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3161-CreateGroupBookingSameAccommodationandDatesThenPayUsingCash");
