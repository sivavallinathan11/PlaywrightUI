...
test('Create a group booking then pay partially using cash', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3168-CreateGroupBookingThenPayPartiallyUsingCash");
