...
test('Create a group booking where all child has an added membership then pay partially using any mode of payment', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3171-CreateGroupBookingAllChildHasMembershipThenPayPartiallyUsingAnyModeofPayment");
