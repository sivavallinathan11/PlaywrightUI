...
import { Accommodation, GroupBookingDataAllMembership, GuestGroupBookingData } from "../../data/users";
...
test('Create a group booking delete one child booking then pay using any mode of payment', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3172-CreateGroupBookingDeleteOneChildBookingThenPayUsingAnyModeofPayment");
    ...
    const customerDetails = GroupBookingDataAllMembership;
    ...
    var bookingDetails = await booking.CreateBookingReservation(customerDetails, 3, "Random", parkDetails);
    ...
    await booking.DeleteAccommodation(bookingDetails, "Any");
    ...
    // Pay using any mode of payment.
    var paymentDetails = await reservepay.ManagePayment(bookingDetails, guestDetails, "Random", "75%");
