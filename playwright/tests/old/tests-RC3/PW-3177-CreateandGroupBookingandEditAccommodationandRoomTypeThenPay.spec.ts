...
import { Accommodation, GuestGroupBookingData } from "../../data/users";
...
test('Create group booking and edit all child booking accommodation and room type then pay', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3177-CreateandGroupBookingandEditAccommodationandRoomTypeThenPay");
    ...
    var bookingDetails = await booking.EditAccommodationandRoomType(bookingDetails, guestDetails);
