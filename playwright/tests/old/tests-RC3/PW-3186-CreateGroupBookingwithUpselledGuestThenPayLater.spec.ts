...
import { Accommodation, GroupBookingDataAllMembership } from "../../data/users";
...
test('Create a group booking with upselled guest then choose to pay later', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3186-CreateGroupBookingwithUpselledGuestThenPayLater");
