...
import { Accommodation, DEV_GuestDatawithWhitelistedEmail, GuestDatawithWhitelistedEmail, TestingEnvironment } from "../../data/users";
...
test('Create a group booking then and check generated email', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3198-CreateGroupBookingThenCheckGeneratedEmail");
