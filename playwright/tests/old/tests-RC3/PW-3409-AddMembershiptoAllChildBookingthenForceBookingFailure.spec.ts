...
test('Add Membership to All Child Booking then force a booking failure @refactored', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3409-AddMembershiptoAllChildBookingthenForceBookingFailure");
