...
import { Accommodation, UpsellGuestData500, UpsellGuestData } from "../data/users";
...
test('Create an individual booking then upsell and pay partially using EFTPOS', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3124-CreateIndividualBookingthenUpsellAndPayPartialUsingEFTPOS");
    ...
    // Pay using EFTPOS partially. If payment has no percentage it will be classified as specific payment.
    var paymentDetails = await reservepay.ManagePayment(bookingDetails, guestDetails, "EFTPOS", "Random");
