...
import { Accommodation, UpsellGuestData500 } from "../data/users";
...
test('Create an individual booking then upsell and pay below $50', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3140-CreateIndividualBookingwithUpselledGuestThenPayBelow50");
    ...
    const customerDetails = UpsellGuestData500;
    ...
    await reservepay.ManagePayment(bookingDetails, guestDetails, "Cash", "45");
