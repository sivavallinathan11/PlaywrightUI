...
import { Accommodation, SameAccommodationAndDatesGroupData } from "../data/users";
...
test('Create a group booking with the same accommodation and dates then pay using eftpos', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3163-CreateGroupBookingSameAccommodationandDatesThenPayUsingEFTPOS");
    ...
    const customerDetails = SameAccommodationAndDatesGroupData;
    ...
    // Pay using eftpos.
    var paymentDetails = await reservepay.ManagePayment(bookingDetails, guestDetails, "EFTPOS");
