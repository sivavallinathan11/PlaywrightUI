...
import { Accommodation, DifferentAccommodationAndDatesGroupData,  } from "../data/users";
...
test('Create a group booking with different accommodation and dates then pay using any mode of payment', async({page, request}) => {
    // Set basesteps object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3164-CreateGroupBookingDifferentAccommAndDatesThenPayUsingAnyModeOfPayment");
    ...
    const customerDetails = DifferentAccommodationAndDatesGroupData;
