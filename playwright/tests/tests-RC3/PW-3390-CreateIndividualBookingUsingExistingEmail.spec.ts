...
import { Accommodation, DEV_GuestEmailwithExistingMember, GuestEmailwithExistingMember, TestingEnvironment } from "../data/users";
...
test('Create and pay an individual booking using cash', async({page, request}) =>{
    ...
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3390-CreateIndividualBookingUsingExistingEmail.spec.ts");
    ...
    if(TestingEnvironment.toLowerCase().trim()=="test"){
        customerDetails = GuestEmailwithExistingMember;
    }
    else{
        customerDetails = DEV_GuestEmailwithExistingMember;
    }
