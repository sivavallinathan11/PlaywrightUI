...
import { Accommodation, BetterOffer, GuestData } from "../data/users";
...
test('Add an offer then assign a non-member to the booking', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3790-AddOfferThenAssignNonMember");
