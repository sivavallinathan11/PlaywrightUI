...
import { Accommodation, BetterOffer, UpsellGuestData } from "../data/users";
...
test('Assign NON-MEMBER guest to booking, join gday membership THEN add an offer that is BETTER THAN GDAY PRICING', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3793-AssignNonMemberAndUpsellThenAddOfferBetterThanGDayPricing");
