...
import { Accommodation, BestMateMember, BetterOffer, DEV_BestMateMember, TestingEnvironment } from "../data/users";
...
test('Add an offer THAT IS BETTER THAN GDAY PRICING then assign an existing member to the booking', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3794-AddOfferBetterThanGDayPricingThenAssignExistingMember");
