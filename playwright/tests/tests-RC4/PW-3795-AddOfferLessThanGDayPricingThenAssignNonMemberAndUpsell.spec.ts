...
test('Add an offer less than Gday Pricing then assign a non-member to the booking the upsell', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3795-AddOfferLessThanGDayPricingThenAssignNonMemberAndUpsell");
