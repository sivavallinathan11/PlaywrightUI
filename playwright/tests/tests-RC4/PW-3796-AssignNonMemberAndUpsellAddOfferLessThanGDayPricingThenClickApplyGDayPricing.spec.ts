...
import { Accommodation, LesserOffer, UpsellGuestData } from "../data/users";
...
test('Assign non-MEMBER and upsell THEN add an offer that is LESS THAN GDAY PRICING the click apply GDay pricing', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3796-AssignNonMemberAndUpsellAddOfferLessThanGDayPricingThenClickApplyGDayPricing");
