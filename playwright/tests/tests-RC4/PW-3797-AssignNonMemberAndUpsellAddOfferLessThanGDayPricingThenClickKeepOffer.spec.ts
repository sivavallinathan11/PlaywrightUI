...
test('Assign non-MEMBER and upsell THEN add an offer that is LESS THAN GDAY PRICING the click keep offer', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3797-AssignNonMemberAndUpsellAddOfferLessThanGDayPricingThenClickKeepOffer");
