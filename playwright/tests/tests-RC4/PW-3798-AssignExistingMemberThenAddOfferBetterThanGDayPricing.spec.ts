...
test('Assign existing MEMBER to booking THEN add an offer that is BETTER THAN GDAY PRICING', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3798-AssignExistingMemberThenAddOfferBetterThanGDayPricing");
