...
import { Accommodation, BetterOffer, GuestDatawithOffers, LesserOffer } from "../data/users";
...
test('Add an offer to a booking then change go back to view offers modal and change the applied offer', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-3815-AddOffersToBookingThenChangeOffers");
    ...
    var afterOfferDetails = await offers.SelectAndProcessOfferFromViewOffersModal(bookingDetails, guestDetails,
        "Accept Offers", BetterOffer, "Better");
    ...
    // Click View offers to change existing offer.
    await booking.ClickViewOffers();

    // Verify view offers is displayed.
    await offers.VerifyViewOffersModal();

    // Select another offer in the view offers modal (Booking Details, Offer Type, Process Type)
    var afterOfferDetails = await offers.SelectAndProcessOfferFromViewOffersModal(bookingDetails, guestDetails,
        "Accept Offers", LesserOffer, "Lesser");
    bookingDetails = afterOfferDetails[1];
    var offerDetails = afterOfferDetails[0];
