...
import { BetterOffer, LesserOffer, MateMember } from "../data/users";
...
test('Add offer less than GDay pricing then assign member to all bookings', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-4324-AddOfferLessThanGdayPricingThenAssignMemberToAllBookings");
