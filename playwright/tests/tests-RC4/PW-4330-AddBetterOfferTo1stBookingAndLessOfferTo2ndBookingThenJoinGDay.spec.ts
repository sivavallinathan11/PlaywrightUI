...
import { BetterOffer, LesserOffer, UpsellGuestData } from "../data/users";
...
test('Add better offer to 1st and lesser offer to 2nd booking then join gday', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-4330-AddBetterOfferTo1stAnd2ndBookingThenJoinGDay");
