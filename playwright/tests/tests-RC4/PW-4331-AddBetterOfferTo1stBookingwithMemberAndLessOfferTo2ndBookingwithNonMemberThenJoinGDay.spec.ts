...
test('Add better offer to a member booking (1st booking) AND Add less offer to a non member booking (2nd booking) then join gday', async({page, request}) =>{
    // Set base object.
    const baseSteps = new BaseSteps();

    // Initialize directory for test.
    const testDetails = await baseSteps.InitializeTestResultDirectory("PW-4331-AddBetterOfferTo1stBookingwithMemberAndLessOfferTo2ndBookingwithNonMemberThenJoinGDay");
    ...
    var newCustomerDetails = [MateMember, UpsellGuestData];
